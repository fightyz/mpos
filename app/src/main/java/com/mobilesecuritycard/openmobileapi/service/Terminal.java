...
public abstract class Terminal implements ITerminal {
   ...
   /**
    ...
    public ChannelAccess enableAccessConditions( PackageManager packageManager, byte[] aid, String[] packageNames, ISmartcardServiceCallback callback){
    	if( mAccessController == null ){
    		Log.d(TERMINAL_SERVICE_TAG, "mAccessController is null, will be created");
    		mAccessController = new AccessController(packageManager);
    	}
    	ChannelAccess ca = null;
    	try {
    	    ca = mAccessController.enableAccessConditions(this, aid, packageNames, callback);
    	} catch(RuntimeException e) {
    	    if(e instanceof NoSuchElementException) {
    	        if(mName.equalsIgnoreCase("SIM: UICC")){
    	            // access to UICC requires ARA-M or ARF to be present
